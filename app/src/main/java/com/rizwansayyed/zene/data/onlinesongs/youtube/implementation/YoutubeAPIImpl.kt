...
//            }
    ...
    override suspend fun songsSuggestionsForSongs(sId: String) = flow {
        ...
        val upNext = youtubeMusicAPI
            .youtubeNextSearchResponse(ytMusicUpNextJsonBody(ip, sId), key)
        ...
        emit(SongsSuggestionsMusic(upNextList, relatedList))
    }.flowOn(Dispatchers.IO)
        ...
        withContext(Dispatchers.IO) {
            val jobs = artists.map { a ->
                async(Dispatchers.IO) {
                    ...
                    try {
                        val searchResponse = youtubeMusicAPI
                            .youtubeSearchResponse(ytMusicArtistsSearchJsonBody(ip, a), key)
                        ...
                        r.contents?.tabbedSearchResultsRenderer?.tabs?.first()?.tabRenderer?.content?.sectionListRenderer?.contents?.forEach { c ->
                            c?.musicShelfRenderer?.contents?.forEach { shelf ->
                                ...
                                val m = MusicData(
                                    thumbnail ?: "", name?.first, artistsN,
                                    name?.second, MusicType.MUSIC
                                )
                                songs.add(m)
                            }
                        }
    ...
    override suspend fun searchMoodPlaylists(q: String) = flow {
        ...
        r.contents?.twoColumnSearchResultsRenderer?.primaryContents?.sectionListRenderer?.contents?.first()?.itemSectionRenderer?.contents?.forEach { c ->
            ...
            val m = MusicData(
                thumbnail ?: "", q.toCapitalFirst(), THE_VIDEO, vId, MusicType.VIDEO
            )
            list.add(m)
        }
        list.shuffle()
